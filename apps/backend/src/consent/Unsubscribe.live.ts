...
export const unsubscribeSuite = (test: LiveTest) => {
  describe("one-click unsubscribe", () => {
    ...
    test(
      "refuses a forged token without opting anybody out",
      Effect.gen(function* () {
        const settings = yield* configuration;
        const unsubscribe = yield* unsubscribeSettings;
        const storage = yield* liveStorage(settings.tableName);
     ...
     */
    test(
      "keeps naming the same mailbox when the contact is edited, deleted and re-imported",
      Effect.gen(function* () {
        const settings = yield* configuration;
        const unsubscribe = yield* unsubscribeSettings;
        const storage = yield* liveStorage(settings.tableName);
        ...
        // behind by moving its contact to another one.
