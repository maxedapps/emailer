...
describe("sendGuard", () => {
  it("takes 80 percent of MaxSendRate, floored, and is not exhausted below 90 percent of Max24HourSend", () => {
    ...
    ).toStrictEqual({ limit: 11, dailyExhausted: false, halted: Option.none() });
    expect(
      sendGuard(
    ...
    ).toStrictEqual({ limit: 11, dailyExhausted: true, halted: Option.none() });
  });

  it("uses 1 when MaxSendRate is undefined", () => {
    expect(
      sendGuard(account({ Max24HourSend: 200, SentLast24Hours: 0 }), silent, undefined),
    ).toStrictEqual({ limit: 1, dailyExhausted: false, halted: Option.none() });
  ...
  it("is never exhausted when Max24HourSend is undefined", () => {
    ...
    ).toStrictEqual({ limit: 8, dailyExhausted: false, halted: Option.none() });
  });
  ...
  it("exhausts at the lower of the 90 percent quota and the daily ceiling", () => {
    ...
    ).toStrictEqual({ limit: 8, dailyExhausted: true, halted: Option.none() });
    expect(
  ...
  it("is halted for alarm when any of four metric alarms is in ALARM", () => {
    expect(
      sendGuard(account(healthy), described(["OK", "ALARM", "INSUFFICIENT_DATA", "OK"]), undefined),
    ).toStrictEqual({ limit: 11, dailyExhausted: false, halted: Option.some("alarm") });
  });
  ...
  it("is halted for enforcement when EnforcementStatus is PROBATION or SHUTDOWN", () => {
    expect(sendGuard(account(healthy, "PROBATION"), silent, undefined)).toStrictEqual({
      limit: 11,
      dailyExhausted: false,
      halted: Option.some("enforcement"),
    });
  ...
  it("reports alarm before enforcement when both hold", () => {
    expect(sendGuard(account(healthy, "PROBATION"), described(["ALARM"]), undefined)).toStrictEqual(
      { limit: 11, dailyExhausted: false, halted: Option.some("alarm") },
    );
  });
