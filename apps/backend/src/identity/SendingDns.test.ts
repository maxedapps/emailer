...
describe("dkimRecords", () => {
  ...
  it.effect("points each token at the identity's own signing zone rather than a fixed one", () =>
    Effect.gen(function* () {
  ...
  it.effect("refuses to guess a zone SES did not report", () =>
    Effect.gen(function* () {
  ...
  it.effect("refuses anything but Easy DKIM's three tokens", () =>
    Effect.gen(function* () {
...
describe("dnsSettings", () => {
  ...
  it.effect(
    "leaves DNS to the operator when no mode is set, including the example file's blank keys",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("reads a provider mode and a normalized report address", () =>
    Effect.gen(function* () {
  ...
  it.effect("dies when a DMARC report address is set without a mode that would publish it", () =>
    Effect.gen(function* () {
      ...
      expect(Result.isSuccess(defect) && defect.success instanceof DmarcRequiresDnsMode).toBe(true);
    }),
  );
});
