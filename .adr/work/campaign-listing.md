...
- **Change:**
  ...
  - `.adr/0005-contact-identity-and-membership-access-paths.md` and `.adr/0008-storage-capabilities-and-error-boundaries.md`: one header line each in the existing lifecycle-line position. No new ADR supersedes them, so the line points at this work document: `- Amended: [campaign-listing](work/campaign-listing.md) — campaign META items join the listing index` and `… — CampaignStore binds all six table operations`.
