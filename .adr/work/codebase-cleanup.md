...
- **Restored — T3.3:** main minted the link inside `Mailer.send`, after the claim. The sync first dropped T3.3, but the user asked for the plan in full, so it came back in main's layout.
  - **Why the Mailer can't mint once:** the link settings are env pinned from the functions' props. Alchemy captures a constructor `Config` read on the deploy machine at plan time, so the Mailer cannot read them when it is built ([Alchemy `Platform.ts`](https://unpkg.com/alchemy@2.0.0-beta.79/src/Platform.ts)).
