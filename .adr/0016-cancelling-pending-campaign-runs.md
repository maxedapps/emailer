...
- Amended: 2026-09-24, on the user's decision in the simplification plan (`work/simplify.md` T5) — no command deletes a schedule, so the generation-specific cleanup, the recheck after creation and cancel's cleanup retries are gone. An outdated schedule fires once as stale and deletes itself, as [ADR-0020](0020-drafts-previews-and-test-sends.md) already accepted for a deleted draft's schedule. Generation-named schedules and token retention stay.
...
- Keep the retired token on draft/paused records; state gates prevent dispatch. Every new send, schedule or resume replaces it with a fresh token. Retention prevents an old draft/paused observation from matching after an intervening lifecycle cycle.
...
- Name each Scheduler resource by its run token inside the existing stage group: an outdated schedule keeps its name until its original time, so a name shared across generations could not be created again before then. Perform the publish or create after the database commit. No command deletes a schedule. A retired generation's fire finds its token replaced, held by a draft or paused campaign, or its campaign deleted, so `beginRun` discards it as stale, and the schedule deletes itself after that one fire.
...
DynamoDB, SQS and Scheduler are not atomic together. A failed publish/create is reported as unavailable and repaired by the existing explicit send/resume/schedule commands. Cancellation touches only the campaign. An outdated schedule, left by a cancel, a send now or a reschedule, stays listed in the stage's schedule group until its original time, then fires once as stale and deletes itself, unless stage teardown removes it with the group first. Until then it counts against the account's schedule quota, and its fire costs one dispatcher invocation that sends nothing. Late creation after cancellation is harmless for the same reason: the worker checks current intent.
