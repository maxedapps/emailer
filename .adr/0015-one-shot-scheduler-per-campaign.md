...
- **Reschedule replaces.** A second `schedule` on a scheduled campaign overwrites `sendAt` under a fresh token and creates a new schedule; the earlier one fires stale, and nothing records the earlier time.
