...
Use `Effect.catchTag` to handle a specific domain error, and `Effect.mapError` to translate SDK details into a stable application contract. Keep the underlying cause available for internal diagnostics without exposing secrets. Do not convert every failure with `Effect.orDie` merely to satisfy an adapter's `never` error channel; decide which failures need retry, quarantine or a public response first. [Error handling examples](https://unpkg.com/effect@4.0.0-rc.117/ai-docs/src/01_effect/04_errors/01_error-handling.ts), [Tagged error recovery](https://unpkg.com/effect@4.0.0-rc.117/ai-docs/src/01_effect/04_errors/10_catch-tags.ts)
...
Prefer `Effect.fn("Service.operation")` for named reusable operations and tracing, or `Effect.gen` for local composition. Services, schemas and durable records should make business invariants explicit; Effect does not automatically persist a computation or make remote side effects idempotent. [ManagedRuntime integration](https://unpkg.com/effect@4.0.0-rc.117/ai-docs/src/04_integration/10_managed-runtime.ts)
