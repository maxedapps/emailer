...
- **A repeated flag:** `Flag.between(min, max)` turns a flag into an array and validates each value through any `Flag.withSchema` applied before it; `Flag.between(0, 20)` makes the flag optional and caps it at twenty. [Flag.ts](https://unpkg.com/effect@4.0.0-rc.117/src/unstable/cli/Flag.ts)
- **No either/or combinator:** nothing in RC117 expresses "exactly one of these flags". Check the combination in the handler and fail with `CliError.UserError({ cause, userMessage })`. The runner prints the `userMessage` to stderr under `ERROR` and exits 1, before the handler has sent any request.
