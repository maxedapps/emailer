...
These articles describe **Alchemy `2.0.0-beta.77` with Effect `4.0.0-rc.112`**. This is a versioned reference, not a floating latest-release recommendation. Beta.77 fails to import with Effect RC113 because it uses a removed `Config.string` constructor, despite a peer range that accepts RC113. Read [version-specific traps](version-specific-traps.md) before combining releases or copying newer examples.
...
If the public documentation site cannot be fetched, retrieve the index, generator and Markdown articles from the official repository. For beta-specific behavior, the [published beta.77 package source](https://unpkg.com/alchemy@2.0.0-beta.77/package.json) takes precedence over an unversioned guide. Its `src` directory contains providers and runtime adapters. Documentation indexes aid discovery; they are not compatibility declarations.
