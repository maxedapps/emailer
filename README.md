...
- `lists import` sends the file 20 contacts per call, 8 calls at a time: about 200 contacts a second into one list, the most DynamoDB takes for one list. A call that fails in transit, times out or answers 408, 429 or 5xx is sent again for about two minutes. Progress goes to stderr every 1,000 contacts. If the import stops, stderr says how many contacts went in; running the same file again completes it.
