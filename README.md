...
| `EMAILER_DAILY_SEND_CEILING` | no       | Positive integer cap on the SES account's sends over the last 24 hours. SES counts the whole account, so every stage and any other sender on it draw from the same cap.                                                               |
